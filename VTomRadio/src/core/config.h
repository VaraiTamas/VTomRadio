...
struct theme_t {
    ...
    /*---- WIFI WIDGET -----*/
    uint16_t wifi_bg;
    uint16_t wifi_border;
    uint16_t wifi_inactive;
    uint16_t wifi_low;
    uint16_t wifi_low_mid;
    uint16_t wifi_mid;
    uint16_t wifi_high;
};
struct config_t {
    uint16_t config_set; // must be 0
    uint16_t version;
    uint8_t  volume;
    int8_t   balance;
    int8_t   trebble;
    ...
    bool     encodersIndependent;
    uint16_t abuff;
    bool     watchdog;
    bool     nameday;
    bool     clockTtsEnabled;
    char     clockTtsLanguage[3];
    uint16_t clockTtsIntervalMinutes;
    bool     clockTtsOnlyWhenNoStream;
    bool     clockTtsQuietHoursEnabled;
    uint16_t clockTtsQuietFromMinutes;
    uint16_t clockTtsQuietToMinutes;
