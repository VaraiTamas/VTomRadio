...
static inline bool hasSecondaryEncoder() {
    return ENC2_BTNL != 255 && ENC2_BTNR != 255;
}

static inline bool useIndependentEncoders() {
    return hasPrimaryEncoder() && hasSecondaryEncoder() && config.store.encodersIndependent;
}
...
myEncoder encoder = myEncoder(ENC_BTNL, ENC_BTNR, ENCODER_STEPS, ENC_INTERNALPULLUP);
#    endif
#    if (ENC2_BTNL != 255 && ENC2_BTNR != 255)
myEncoder encoder2 = myEncoder(ENC2_BTNL, ENC2_BTNR, ENCODER2_STEPS, ENC2_INTERNALPULLUP);
#    endif
#endif
...
void initControls() {
    ...
    for (int i = 0; i < nrOfButtons; i++) {
        ...
        button[i].attachLongPressStart([](void* p) { onBtnLongPressStart((int)p); }, (void*)i);
        button[i].attachLongPressStop([](void* p) { onBtnLongPressStop((int)p); }, (void*)i);
...
void encodersLoop(myEncoder* enc, bool first) {
    ...
    if (checklpdelay(BTN_LONGPRESS_LOOP_DELAY, lpdelay)) {
        switch ((controlEvt_e)id) {
            ...
            case EVT_BTNDOWN: {
                ...
                if (display.mode() == STATIONS) { controlsEvent(id == EVT_BTNDOWN); }
                break;
            }
            default: break;
        }
...
void controlsEvent(bool toRight, int8_t volDelta) {
    ...
    if (display.mode() != STATIONS) {
#if !defined(DUMMYDISPLAY) || defined(USE_NEXTION)
        Serial.println("controlsEvent: volume change");
